...
class TargetSelector
{
    ...
    private Unit GetLowestHPTarget()
    {
        Unit lowesttarget = null;
        foreach (Unit target in Unit.Heroes)
        {
            if (target != null && target.IsValid == true && target.IsEnemy == true && target.IsDead == false && target.IsVisible == true && Vector3.Distance(Unit.Player.Position, target.Position) <= _range)
            {
    ...
    private Unit Closest()
    {
        ...
        foreach (Unit target in Unit.Heroes)
        {
            if (target != null && target.IsValid == true && target.IsEnemy == true && target.IsDead == false && target.IsVisible == true && Vector3.Distance(Unit.Player.Position, target.Position) <= _range)
            {
                ...
                {
                    if (Vector3.Distance(Unit.Player.Position, target.Position) < Vector3.Distance(Unit.Player.Position, closest.Position))
                    {
                        closest = target;
                    }
                }
    ...
    private Unit AutoPriority()
    {
        Unit autopriority = null;
        int prio = 5;
        foreach (Unit target in Unit.Heroes)
        {
            if (target != null && target.IsValid == true && target.IsEnemy == true && target.IsDead == false && target.IsVisible == true && Vector3.Distance(Unit.Player.Position, target.Position) <= _range)
    ...
    public void OverrideTarget(Unit newtarget)
    {
        target = newtarget;
        update = false;
    }
